...
/// </summary>
/// <typeparam name="TResolveFailure"></typeparam>
public class ByteArrayMemoryAdapter<TResolveFailure> : IMemoryAdapter<byte[], ReadFailure, WriteFailure>
{
    private readonly IAddressResolver<TResolveFailure> _addressResolver;
    ...
    /// <typeparam name="TResolveFailure"></typeparam>
    public ByteArrayMemoryAdapter(IAddressResolver<TResolveFailure> addressResolver, int size)
    {
        if (size <= 0)
    ...
    public Result<byte[], ReadFailure> Read(ProcessMemory processMemory)
    {
    ...
    public Result<WriteFailure> Write(ProcessMemory processMemory, byte[] value)
    {
        if (value.Length != _size)
            return new WriteFailureOnInvalidArguments(
